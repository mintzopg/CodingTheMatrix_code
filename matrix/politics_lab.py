...
def bitter_rivals(voting_dict):
    ...
    """
    difference={}
    for senator in voting_dict.keys(): # for each senator
        least=least_similar(senator, voting_dict) # find least similar
        policy=policy_compare(senator, least, voting_dict) # compare policies
        difference[(senator,least)]=policy # populate difference dict. {(name1,name2):dot_prod}
    minimum=min(difference.values()) # min dot_prod =>great difference
    for k,v in difference.items(): # search the dict to dinf the key corresponding to the minimum dot_prod.
        if v==minimum:
            return k # return tuple of names
